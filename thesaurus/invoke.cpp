...
auto foo(F f, _Ty1 arg1, _Types2&&... args) -> std::enable_if_t<
    std::is_member_function_pointer<F>::value and
    not std::is_pointer<_Ty1>::value
>
{
    cout_dump();
...
int main(int argc, char** argv) {
    ...
    foo(mp_ops_int, b, 100);
    foo(mp_ops_2int, &b, 200, 300);
}
